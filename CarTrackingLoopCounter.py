    ...
    while True:
        ...
        for index, row in detections.iterrows():
            if row['name'] in ['car', 'truck']:  # Consider 'car' and 'truck' classes
                ...
                car_id = (int(row['xmin']), int(row['ymin']), int(row['xmax']), int(row['ymax']))
                ...
                cv2.putText(frame, f"{row['name']} {row['confidence']:.2f}", (int(row['xmin']), int(row['ymin'])-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)

        # Update tracked cars list
        tracked_cars.update({car: True for car in current_frame_cars})

        # Display count
        cv2.putText(frame, f'Vehicles Counted: {car_counter}', (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
